...
export class UI {
  ...
  settings() {
    ...
    this.frame(
      ...
      `<div class="settings">${[
        ...
        .join(
          '',
        )}<label><span>GRAPHICS QUALITY</span><select data-setting="quality"><option value="low" ${s.quality === 'low' ? 'selected' : ''}>LOW / NO SHADOWS OR POST</option><option value="medium" ${s.quality === 'medium' ? 'selected' : ''}>MEDIUM / BLOOM + GRADE</option><option value="high" ${s.quality === 'high' ? 'selected' : ''}>HIGH / + AMBIENT OCCLUSION</option></select></label><label><span>SHADOWS</span><input type="checkbox" data-toggle="shadows" ${s.shadows !== false ? 'checked' : ''}></label><label><span>PARTICLE EFFECTS</span><input type="checkbox" data-toggle="particles" ${s.particles !== false ? 'checked' : ''}></label><h3>ACCESSIBILITY / FIELD ASSIST</h3>${[
        ...
        .join(
          '',
        )}<p class="note">Reduced motion removes camera bob, random shake, sprint FOV changes, and muzzle flashes. Weapon recoil remains part of combat. Reduced motion also removes movement camera tilt, landing dips, FOV kicks and speed blur. Low quality reduces resolution and disables shadows and post-processing; resolution also scales down automatically if the frame rate drops. Settings, alloy, unlocks and operation records are stored on this browser.</p><button class="subtle" data-action="reset">RESET LOCAL RECORD & SETTINGS</button><div id="save-message" role="status"></div></div>`,
    );
  ...
  update(dt) {
    ...
    n['announcement'].classList.toggle('below-boss', !!boss);
    if (boss) {
